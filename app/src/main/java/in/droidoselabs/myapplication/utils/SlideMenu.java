...
public class SlideMenu extends ViewGroup {
    ...
    public SlideMenu(Context context, AttributeSet attrs, int defStyle) {
        ...
        setEdgetSlideWidth(a.getDimensionPixelSize(
                R.styleable.SlideMenu_edgeSlideWidth, i));
        a.recycle();
    ...
    protected void resolveSlideMode() {
        ...
        if(mSlideMode == MODE_SLIDE_WINDOW){
            Log.e("Mode", "SLIDE WINDOW");
        ...
        }else if(mSlideMode == MODE_SLIDE_CONTENT){
            Log.e("Mode", "SLIDE CONTENT");
    ...
    public void smoothScrollContentTo(int targetOffset, float velocity) {
        ...
        duration = val1;
        if (velocity > 0) {
            duration = 3 * Math.round(val2 * Math.abs(distance / velocity));
        }
